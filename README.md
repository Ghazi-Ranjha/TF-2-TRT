...
- **minimum_segment_size**: This parameter determines the minimum number of TensorFlow nodes in a TF-TRT engine. Subgraphs with fewer nodes than this number will not be converted to TensorRT. Smaller numbers, such as 5, are preferred for better optimization. It can also be used to change the minimum number of nodes in the optimized INT8 engines to fine-tune result accuracy.

- **max_workspace_size_bytes**: TF-TRT operators often require temporary workspace. This parameter limits the maximum size that any layer in the network can use. If insufficient scratch is provided, TF-TRT may not find an implementation for a given layer.
